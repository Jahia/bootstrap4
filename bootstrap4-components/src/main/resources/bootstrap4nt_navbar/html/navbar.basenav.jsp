...
<template:addResources type="inline">
    <style>.navbar .btn-link.nav-link{color:inherit;text-decoration:none;border:0;background:transparent;}</style>
                ...
                <c:choose>
                    <c:when test="${hasLevel2Pages && recursive}">
                        <li class="nav-item  ${page1Active? ' active' :''} dropdown">
                            <button class="btn btn-link nav-link dropdown-toggle ${page1Active? ' active' :''}"
                                    type="button"
                                ...
                                <c:forEach items="${level2Pages}" var="level2Page" varStatus="status">
                                    ...
                                    <c:if test="${displayLevel2Page}">
                                        ...
                                        <a class="dropdown-item ${page2Active? ' active' :''}"
                                           href="${page2Url}"
                                           <c:if test="${page2Active}">aria-current="page"</c:if>>${fn:escapeXml(page2Title)}</a>
                                    </c:if>
                    ...
                    <c:otherwise>
                        <li class="nav-item ${page1Active? ' active' :''}">
                            <a class="nav-link" href="${page1Url}"
                               <c:if test="${page1Active}">aria-current="page"</c:if>>${fn:escapeXml(page1Title)}</a>
                        </li>
                    </c:otherwise>
                </c:choose>
            </c:if>
